...
async def change_force_sub_mode(client: Client, message: Message):
    temp = await message.reply("<b><i>Just a sec..</i></b>", quote=True)
    channels = await db.show_channels()

    if not channels:
        return await temp.edit("<b>You haven't added any channel for force subscribe. 🤬</b>")
    ...
    await temp.edit(
        "<b>⚡ Select a channel for Force Subscribe:</b>",
...
async def add_force_sub(client: Client, message: Message):
    ...
    try:
        ...
        return await temp.edit(f"❌ Failed to add channel:\n<code>{chat_id}</code>\n\n<i>{e}</i>")
